 ...
 */
export const initFirebase = (options) => {
  if (getApps().length) {
 ...
 */
export const handshakeFirebase = async (setterCallback) => {
 ...
 */
export const only_if_cloudConnected = (callback) => {
 ...
 */
export const removeDeviceFromCloud = async (deviceUuid) => {
  try {
 ...
 */
export const removeDeviceFromDevicesToSync = async (deviceUuid) => {
  try {
    ...
    for (var i = 0; i < devices.data.length; i++) {
      const devicesToSyncExceptRemoved = devices.data[i].devicesToSync.filter((deviceToSync) => deviceToSync != deviceUuid);
 ...
 */
export const getAllElementsInCloud = async ({ collection, queryConstraints }) => {
  const EMPTY_OBJECT = {
    isPresent: false,
    data: [],
  };
 ...
 */
export const getElementInCloud = async ({ collection, identifier }) => {
 ...
 */
export const setElementInCloud = async ({ collection, identifier, payload, merge = false, noUpdateLastSync = false }) => {
  const { db } = retrieveFirebase();
 ...
 */
export const deleteElementInCloud = async ({ collection, identifier, noUpdateLastSync = false }) => {
  const { db } = retrieveFirebase();
 ...
 */
export const getCollectionIdentifiersInCloud = async (collection) => {
  const { db } = retrieveFirebase();
  if (db == null) {
    return null;
  }
 ...
 */
export const deleteCollectionInCloud = async (collection) => {
  const { db } = retrieveFirebase();
