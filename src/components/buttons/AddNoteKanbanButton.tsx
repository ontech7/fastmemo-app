...
export default function AddNoteKanbanButton({ isDeleteMode, toggleDeleteMode, style = {} }: Props) {
  ...
  return (
    <Animated.View style={{ transform: [{ scaleX: scaleInterpolation }] }}>
      <TouchableOpacity
        ...
        onPress={() => {
          if (!isDeleteMode) {
            router.push("/notes/new-kanban");
          } else {
            toggleDeleteMode();
          }
        }}
      >
        <ViewColumnsIcon size={24} color={COLOR.darkBlue} />
      </TouchableOpacity>
    </Animated.View>
  );
}
